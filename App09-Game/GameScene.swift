...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    init(_ score: Binding<Int>) {
        _currentScore = score
        super.init(size: CGSize(width: 926, height: 460))
        self.scaleMode = .fill
    }
...
//        plane.position = location
    ...
    func parallaxScroll(image: String, y: CGFloat, z: CGFloat, duration: Double, needsPhysics: Bool) {
        ...
        for i in 0 ... 1 {
            ...
            //node.size = CGSize(width: 926, height: 460)
            node.zPosition = z
            addChild(node)
    ...
    @objc func createObstacle() {
        ...
        let rand = GKRandomDistribution(lowestValue: -130, highestValue: 170)
        ...
        let collision = SKSpriteNode(color: UIColor.clear, size: CGSize(width: 20, height: 460))
